...
def read_demultiplex_metrics(
    ...
    for path in paths:
        ...
        if raw.get("processing_mode") == "already_split":
            library_id = _required_text(
                raw, "library_id", label="demultiplex metrics"
            )
            sample_id = _required_text(
                raw, "sample_id", label="demultiplex metrics"
            )
            if metadata is None or sample_id not in metadata:
                raise DashboardInputError(
                    f"already-split demultiplex metrics reference unknown sample_id {sample_id}"
                )
            if _required_text(
                metadata[sample_id], "library_id", label=f"metadata {sample_id}"
            ) != library_id:
                raise DashboardInputError(
                    f"already-split demultiplex metrics library_id does not match {sample_id}"
                )
            expected_sample_ids = {
                current_id
                for current_id, record in metadata.items()
                if _required_text(
                    record, "library_id", label=f"metadata {current_id}"
                ) == library_id
            }
            if expected_sample_ids != {sample_id}:
                raise DashboardInputError(
                    f"already-split library {library_id} must contain only {sample_id}"
                )
    ...
    *,
    ...
    for sample_id, sample in samples_by_id.items():
        ...
        if demux is None:
            ...
            sample["demultiplex"]["processing_mode"] = demux.get(
                "processing_mode", "i2_demultiplexed"
            )
            if demux.get("processing_mode") == "already_split":
                _set_availability(
                    sample, "demultiplex", "skipped", "already_split"
                )
            else:
                ...
                sample["demultiplex"].update({
                    ...
                    "sample_assignment_fraction": (
                        sample_count / total_assigned
                        if sample_count is not None and total_assigned
                        else None
                    ),
                })
