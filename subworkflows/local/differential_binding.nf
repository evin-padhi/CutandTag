...
workflow DIFFERENTIAL_BINDING {
    ...
    target_peaks = final_peaks.map { meta, peaks ->
        tuple(meta.sample_id.toString(), meta.assay_target.toString(), peaks)
    }

    paired_targets = target_bams.join(target_peaks).map {
        sample_id, bam_assay, bam, peak_assay, peaks ->
            ...
            tuple(bam_assay, sample_id, bam, peaks)
    }

    assay_groups = paired_targets
        .groupTuple(by: 0)
        .map { assay_target, raw_sample_ids, raw_bams, raw_peaks ->
    ...
    peak_inputs = assay_groups.map { assay_target, sample_ids, bams, peaks ->
        tuple(assay_target, sample_ids, peaks)
    }
    bam_inputs = assay_groups.map { assay_target, sample_ids, bams, peaks ->
