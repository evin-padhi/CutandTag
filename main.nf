...
workflow NANOCUT {
    ...
    motif_ame_statuses_ch = Channel.empty()
    if (validated.motif_db != null) {
        ...
        motif_ame_statuses_ch = MOTIFS.out.statuses
            .filter { meta, stage, status -> stage == 'ame' }
            .map { meta, stage, status -> tuple(meta, status) }
